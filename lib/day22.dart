...
class Day22 extends StatelessWidget {
  ...
  int doPart1() {
    ...
    Set<int> xPoints = cubes.getOrderedSetOfXPoints();
    Set<int> yPoints = cubes.getOrderedSetOfYPoints();
    Set<int> zPoints = cubes.getOrderedSetOfZPoints();
    // Sweep from lowest Z
    for (int zLevel in zPoints) {
      for (int xLevel in xPoints) {
        for (int y in yPoints) {
          debugPrint(' $xLevel, $y, $zLevel');
        }
      }
    }
    return 0;
...
class CubeSpace {
  ...
  void handleCommand(Command command) {
    ...
    for (Cube cube in cubeList) {
      if (cube.interSect(newCube)) {
        final splitCubes = split(cube, newCube); // Split original cube
        final newSplitCubes = split(newCube, cube);
        if (newCube.positive) {
          newCubeList.addAll(addPositiveCubes(splitCubes, newSplitCubes));
        } else {
          List<Cube> remains = removeNegativeCubes(splitCubes, newSplitCubes);
          newCubeList.addAll(remains);
        }
      } else {
  ...
  List<Cube> split(Cube cubeToSplit, Cube cubeForSplit) {
  ...
  List<Cube> splitInX(Cube cube, List<int> splitAtX) {
    // Split in X plane
    List<Cube> result = [];
    if (splitAtX.isEmpty) {
      result.add(cube);
    ...
    if (splitAtX.length == 2) {
      result.add(Cube(cube.positive, cube.xMin, splitAtX[0], cube.yMin,
          cube.yMax, cube.zMin, cube.zMax));
  ...
  List<Cube> splitInY(Cube cube, List<int> splitAtY) {
    // Split in Y plane
    List<Cube> result = [];
    if (splitAtY.isEmpty) {
      result.add(cube);
    ...
    if (splitAtY.length == 2) {
      ...
      result.add(Cube(cube.positive, splitAtY[1], cube.xMax, splitAtY[1],
          cube.yMax, cube.zMin, cube.zMax));
    }
    return result;
  }

  List<Cube> splitInZ(Cube cube, List<int> splitAtZ) {
    // Split in Z plane
    List<Cube> result = [];
    if (splitAtZ.isEmpty) {
      result.add(cube);
  ...
  Set<int> getOrderedSetOfXPoints() {
    SplayTreeSet<int> xPointSet = SplayTreeSet();
    for (final cube in cubeList) {
      xPointSet.add(cube.xMin);
      xPointSet.add(cube.xMax);
    }
    for (final x in xPointSet) {
      debugPrint('x : $x');
    }
    return xPointSet;
  }

  Set<int> getOrderedSetOfYPoints() {
    SplayTreeSet<int> yPointSet = SplayTreeSet();
    for (final cube in cubeList) {
      yPointSet.add(cube.yMin);
      yPointSet.add(cube.yMax);
    }
    for (final y in yPointSet) {
      debugPrint('y : $y');
    }
    return yPointSet;
  }

  Set<int> getOrderedSetOfZPoints() {
    SplayTreeSet<int> zPointSet = SplayTreeSet();
    for (final cube in cubeList) {
      zPointSet.add(cube.zMin);
      zPointSet.add(cube.zMax);
    }
    for (final z in zPointSet) {
      debugPrint('z : $z');
    }
    return zPointSet;
  }

  // Boolean add positive cubes
  List<Cube> addPositiveCubes(List<Cube> cubeSet1, List<Cube> cubeSet2) {
    for (final cubeToAdd in cubeSet2) {
      bool addCube = true;
      for (final cube in cubeSet1) {
        if (cube.isEqualSize(cubeToAdd)) addCube = false;
      }
      if (addCube) cubeSet1.add(cubeToAdd);
    }

    return cubeSet1;
  }

  // Remove negative cubes
  List<Cube> removeNegativeCubes(List<Cube> cubeSet1, List<Cube> cubeSet2) {
    List<Cube> resultList = [];
    for (final cube in cubeSet1) {
      bool okToAdd = true;
      for (final cubeToRemove in cubeSet2) {
        if (!cube.isEqualSize(cubeToRemove)) okToAdd = false;
      }
      if (okToAdd) resultList.add(cube);
    }
    return resultList;
  }
